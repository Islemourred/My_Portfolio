...
function ServiceCard({ title, description, icon: Icon, color, index }) {
  return (
    <motion.div
      ...
      className={`w-full max-w-xs rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300 p-7 flex flex-col items-center text-center border border-gray-100 mx-auto ${color}`}
    >
      ...
      <h3 className="text-blue-900 font-bold text-xl mb-2 text-center">
        {title}
      </h3>
      <p className="text-dark-gray text-base mb-1 text-center">{description}</p>
    </motion.div>
  );
}
